...
public class SpellCastSpawner(World world) : Manipulator(world)
{
    public void SpawnFireball(int x, int y, GridDirection gridDirection)
    {
        ...
        SetProjectileComponents(fireball, 1, ProjectileMoveRate.Immediate, gridDirection, Constants.DEFAULT_PROJECTILE_SPEED);
    }
    ...
    public void SpawnArcaneBubble(int x, int y, GridDirection gridDirection)
    {
        ...
        SetProjectileComponents(arcaneBubble, 2, ProjectileMoveRate.PerStep, gridDirection, Constants.DEFAULT_PROJECTILE_SPEED/10);
    }

    private void SetProjectileComponents(Entity entity, int damage, ProjectileMoveRate moveRate, GridDirection gridDirection, int speed)
    {
        world.Set(entity, new ProjectileDamageComponent(damage));
        world.Set(entity, new ProjectileMoveRateComponent(moveRate));
        world.Set(entity, new MovingInDirectionComponent(gridDirection));
